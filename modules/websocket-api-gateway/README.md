...
- Access logging and route settings are opt-in via `enable_api_gateway_logs` (default `false`). `stage_access_log_settings` is passed as `null` to the upstream `terraform-aws-modules/apigateway-v2` module when `false`, so no log group is created and access logging stays off. `stage_default_route_settings` is always passed as an object (never `null`), with `data_trace_enabled`/`detailed_metrics_enabled` forced to `false` and `logging_level` forced to `"OFF"` when `enable_api_gateway_logs` is `false` — passing `null` there previously left route-level logging settings unset instead of explicitly disabled. Passing `{}` for `stage_access_log_settings` would **not** disable logging — the upstream module treats a non-null object as enabled and defaults `create_log_group` to `true`.
